...
class Graceful429Plugin(BasePlugin):
    ...
    def apply_test_failover(self, agent):
        ...
        def collect_targets(current_agent):
            # Recurse into sub-agents recursively 
            if hasattr(current_agent, 'sub_agents') and current_agent.sub_agents:
                for s in current_agent.sub_agents:
                    collect_targets(s)
            
            # Append model for the current agent
            if hasattr(current_agent, 'model') and current_agent.model:
                targets.append(current_agent.model)
                if hasattr(current_agent.model, 'client'):
                    targets.append(current_agent.model.client)

        collect_targets(agent)
